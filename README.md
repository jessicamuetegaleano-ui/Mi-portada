<img width="173" height="233" alt="image" src="https://github.com/user-attachments/assets/1d8d3949-eca6-49df-8287-9db8392588cb" />

#📊 Jessica Alejandra Muete Galeano
### Analista de Datos Junior | Data Analytics | Business Intelligence

---

## 👋 Acerca de mí

**Analista de Datos Junior** con experiencia previa en áreas administrativas y operativas, aportando capacidad para organizar información y optimizar procesos. 

Soy una profesional orientada al **detalle** con formación en:
- 📈 Manejo de bases de datos
- 🔍 Análisis de información 
- 💡 Generación de insights claros para la toma de decisiones

Actualmente en proceso de aprendizaje de **Python, Pandas y SQL** para fortalecer mi análisis estadístico y automatización de tareas.

### Mi enfoque
Tengo habilidades de **comunicación efectiva**, **trabajo en equipo** y **resolución de problemas** en entornos dinámicos. Aprendo rápidamente y busco aportar valor con análisis que generen impacto en la toma de decisiones.

---

## 🛠️ Habilidades Técnicas

### Programación & Análisis
- **Lenguajes:** Python | SQL
- **Librerías:** Pandas | Matplotlib
- **Bases de Datos:** SQL | Gestión de datos

### Herramientas de Datos
- **Hojas de Cálculo:** Excel | Google Sheets
- **Visualización:** Power BI | Matplotlib
- **Análisis:** KPIs | Análisis de datos | Generación de insights

### Competencias Blandas
- 🧠 Pensamiento analítico
- 👁️ Atención al detalle
- 💬 Comunicación efectiva
- 🤝 Trabajo en equipo

---

## 📁 Proyectos Destacados

### 1. **Análisis del Desempeño Financiero - Adventure Works**
Análisis financiero completo de empresa ficticia Adventure Works
- 📊 Análisis de ingresos y rentabilidad por mercados
- 🎯 Optimización de asignación de recursos
- 💼 Estrategia de inversión en marketing
- 🔗 [Ver Proyecto](https://github.com/jessicamuetegaleano-ui/Analisis-del-desempeno-financiero-)

### 2. **Movilidad Urbana y Productividad Económica**
Análisis de infraestructura de transporte e impacto económico
- 🚗 Identificación de ciudades clave para inversión
- 📍 Mejora de productividad y bienestar poblacional
- 📊 Análisis comparativo de ciudades
- 🔗 [Ver Proyecto](https://github.com/jessicamuetegaleano-ui/Movilidad-urbana-y-productividad-economica)

---

## 🎯 Objetivo Profesional

Me interesa **iniciar mi carrera como Analista de Datos Junior**, donde pueda:
- ✅ Seguir desarrollando mis habilidades técnicas
- ✅ Contribuir con análisis que generen impacto
- ✅ Aprender de profesionales experimentados
- ✅ Aportar valor a través de insights claros

---

## 🌟 Intereses Personales

En mi tiempo libre disfruto:
- 🎬 Películas basadas en historias reales y documentales de crímenes
- 👨‍🍳 Cocinar y experimentar nuevas recetas
- 📚 Leer novelas románticas
- 💻 Aprender nuevas herramientas tecnológicas
- 📈 Fortalecer mis habilidades en análisis de datos
- 🚀 Mantenerme en constante crecimiento profesional

---

## 📫 Contacto

¿Hablamos? Puedes encontrarme en:

📧 **Email:** [jessicamuetegaleano@gmail.com](mailto:jessicamuetegaleano@gmail.com)

💼 **LinkedIn:** [Jessica Alejandra Muete Galeano](https://linkedin.com/in/jessica-alejandra-muete-galeano-9b2b7621a)

📱 **GitHub:** [@jessicamuetegaleano-ui](https://github.com/jessicamuetegaleano-ui)

---

## 📊 Estadísticas

![GitHub stats](https://github-readme-stats.vercel.app/api?username=jessicamuetegaleano-ui&show_icons=true&theme=radical)

---

⭐ _"Los datos cuentan historias. Mi objetivo es descubrirlas y comunicarlas de manera clara y accionable."_
